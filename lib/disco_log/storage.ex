defmodule DiscoLog.Storage do
  ...
  def handle_continue(
    ...
    existing_threads = get_occurrence_threads(discord_client, guild_id, occurrences_channel_id)
    existing_tags = get_occurrence_tags(discord_client, occurrences_channel_id)
  ...
  defp get_occurrence_threads(discord_client, guild_id, occurrences_channel_id) do
    {:ok, %{body: %{"threads" => threads}}} = API.list_active_threads(discord_client, guild_id)
  ...
  defp get_occurrence_tags(discord_client, occurrences_channel_id) do
    {:ok, %{body: %{"available_tags" => available_tags}}} =
      API.get_channel(discord_client, occurrences_channel_id)

    for %{"id" => id, "name" => name} <- available_tags, into: %{}, do: {name, id}
  end
