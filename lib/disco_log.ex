defmodule DiscoLog do
    ...
    with :ok <- maybe_dedupe(error, config) do
      ...
      |> case do
        nil ->
          ...
          message = DiscoLog.Prepare.prepare_occurrence(error, applied_tags)
        ...
        thread_id ->
          message = DiscoLog.Prepare.prepare_occurrence_message(error)
  ...
  def log_info(message, metadata, config) do
    message = DiscoLog.Prepare.prepare_message(message, metadata)
